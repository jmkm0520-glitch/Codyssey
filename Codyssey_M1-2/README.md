...
첨부 평가표를 기준으로 보면, M1-2의 핵심은 **데이터를 저장하고 요약하는 웹 서비스에 AI 대화를 연결하고, 그 설계 이유까지 설명하는 것**입니다. 이 프로젝트는 그 주제를 온라인 쇼핑몰의 일별 매출로 정했습니다. 화면만 만드는 것으로 끝나지 않고, 브라우저에서 입력한 값이 API를 거쳐 데이터베이스에 남고, 그 데이터를 근거로 AI가 답하며, 이전 대화까지 다시 불러올 수 있어야 합니다.

매출 합계·평균·증감률은 Python 코드가 계산하고, AI는 계산된 결과를 사람이 이해하기 쉬운 문장으로 설명합니다. 직접 AI 모델을 학습시키거나 미래 매출을 예측하는 기능은 구현하지 않았습니다. 평가표 상단의 “이번 달 실적”과 원화 금액은 대화 예시이며, 이 구현의 데이터는 과거 영국 소매 매출이고 단위는 GBP입니다. 따라서 현재 달 실적이나 월별 최고 실적을 이미 제공한다고 설명하면 안 됩니다.
...
CORS는 프론트와 백엔드의 출처가 다를 때 브라우저가 응답 접근을 제한하는 정책입니다. 로컬의 `localhost:3000`과 `localhost:8000`도 포트가 달라 다른 출처이며, Vercel과 Render도 도메인이 다릅니다. 백엔드 `ALLOWED_ORIGINS`에 실제 프론트 출처를 등록하면 `CORSMiddleware`가 필요한 허용 헤더와 사전 요청 처리를 제공합니다. 이 저장소만으로 과거 장애의 원인이나 현재 Render 설정값을 확정할 수는 없으므로, 평가에서는 구현된 설정 방식과 실제 배포 설정을 구분해 설명합니다.
...
아래 표는 첨부 평가문항을 구현 근거와 연결한 설명입니다. 통과 여부를 표시한 표는 아니며 배포 상태와 실제 동작은 평가 시 확인해야 합니다.

| 평가 항목 | 설명·확인할 내용 | 관련 문서·코드 |
| --- | --- | --- |
| 항목 1 · 배포 URL 접속 | 사용자 화면이 열리고 API 연결 후 주요 영역이 표시되는지 | [배포 주소](#배포-주소), `app.js` |
| 항목 1 · Swagger UI | `/docs`에서 API 목록·요청 스키마·응답을 볼 수 있는지 | [API 목록](#api-목록), `backend/main.py` |
| 항목 1 · 데이터 저장·목록 반영 | 추가 성공 후 목록·요약 재조회, 새로고침 후에도 값 유지, Firestore 문서와 일치 | `data.js`, `data_repository.py` |
| 항목 1 · 요약·AI 답변 연계 | 요약 응답의 최대 매출·날짜 등과 AI 답변을 비교 | 위 요약·컨텍스트 주입 설명 |
| 항목 1 · 대화 저장·불러오기 | 질문·답변 성공 후 목록에 나타나고 선택 시 같은 메시지가 복원되는지 | 위 대화 저장·화면 상태 설명 |
| 항목 1 · 모바일 | 좁은 화면에서 입력·전송·대화 선택·삭제가 가능한지 | `styles.css`: 기본 세로 배치, 640px 이상 가로 배치, 900px 이상 헤더 조정 |
| 항목 2 · 라우터/서비스 분리 | URL·오류 처리 / 계산·흐름 / DB 접근으로 책임 분리 | 위 계층 분리 설명 |
| 항목 2 · 컬렉션 설계 | 날짜 ID의 일별 문서와 자동 ID의 대화 문서가 필요한 이유 | [Firestore 데이터 구조](#firestore-데이터-구조), 위 저장 구조 설명 |
| 항목 2 · Pydantic | 입력과 출력 모델의 목적, 제약, 검증 위치 | 위 스키마 설명 |
| 항목 2 · 프론트 상태 | 데이터 갱신, 선택 대화 ID, 전송 상태가 연결되는 방식 | 위 화면 상태 표 |
| 항목 3 · 컨텍스트 주입 | 요약을 시스템 메시지에 넣는 이유와 정보 누락·환각 한계 | 위 컨텍스트 주입 설명 |
| 항목 3 · 별도 요약 API | 화면 재사용·계산 책임 분리, 채팅과 공통 함수 공유 | 위 요약 API 설명 |
| 항목 3 · 대화 저장 선택 | AI 성공 후 질문·답변을 함께 저장하는 이유와 배열 방식의 장단점 | 위 대화 저장 설명 |
| 항목 3 · 환경변수 | 백엔드 비밀값·환경별 설정 분리, 정적 프론트 설정 방식 | [환경변수 목록](#환경변수-목록), [백엔드 실행](#백엔드-실행) |
| 항목 4 · 콜드스타트 | 지연 안내·연결 배너·재시도 버튼과 15초 제한의 한계 | 위 콜드스타트 설명 |
| 항목 4 · CORS | 출처 차이의 의미와 허용 프론트 출처 설정 | 위 CORS 설명 |
| 항목 4 · 입력 위험 | 형식 검증·출력 이스케이프·프롬프트 안내와 미구현 보호 기능 | 위 입력 위험 설명 |
| 항목 4 · 확장·기준 변경 | 범위 조회와 집계 개선, 달력 기간 정의, 수정할 코드 | 위 최근 30일 변경 설명 |
| 항목 5 · 보너스 | 첨부 표에는 “보너스 문제 해결에 따른 크레딧 부여”만 있어 구체 조건을 판단할 수 없음 | 별도 보너스 문제 내용이 있어야 충족 여부 설명 가능 |

평가에서 전체를 짧게 설명한다면 다음처럼 연결할 수 있습니다.

> 이 과제는 저장한 데이터를 근거로 답하는 AI 웹 서비스를 만드는 과제입니다. 저는 일별 쇼핑몰 매출을 Firestore에 저장하고 FastAPI에서 요약한 뒤, 그 요약을 시스템 프롬프트에 넣어 AI가 설명하도록 했습니다. 라우터는 요청과 응답, 서비스는 계산과 채팅 흐름, 저장소는 DB 접근을 맡습니다. 화면은 저장 성공 후 목록과 요약을 다시 조회하고, 채팅 성공 시 받은 대화 ID로 기록을 이어갑니다. 대화는 답변 성공 후 질문과 답변을 함께 저장합니다. 입력 검증과 HTML 이스케이프, 첫 접속 안내, CORS 설정을 적용했지만 인증과 긴 대화 처리 등은 개선이 필요합니다.
